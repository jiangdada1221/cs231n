...
class TwoLayerNet(object):
    ...
    #     '''
    #     out = np.exp(X)  #N * C
    #     sumOfcol = np.sum(out,axis=1)  #(N,)
    #     for i in range(X.shape[0]):
    #         out[i] = out[i] / sumOfcol[i]
    #     return out
    ...
    def loss(self, X, y=None):
        ...
        da1,dw2,db2 = affine_backward(da2,cache3)
        da1 = relu_backward(da1,cache2)
        dx,dw1,db1 = affine_backward(da1,cache1)
        dw1,dw2 = dw1 + self.reg * W1, dw2 + self.reg*W2
        grads['W1'] = dw1
        grads['W2'] = dw2
        grads['b2'] = b2
        grads['b1'] = b1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if self.normalization=='batchnorm':
            ...
            for i in range(self.num_layers - 1):
                self.bn_params[i] = {'mode':'train','eps':1e-5,'momentum':0.9,'running_mean':np.zeros(hidden_dims[i]),
    ...
    def loss(self, X, y=None):
        ...
        input = X
        for layer in range(self.num_layers):
            w,b = self.params.get('W'+str(layer+1)),self.params.get('b'+str(layer+1))
            out,cache1 = affine_forward(input,w,b)
            if layer != self.num_layers-1:
                if self.normalization == 'batchnorm':
                    gamma,beta = self.params.get('gamma'+str(layer+1)),self.params.get('beta'+str(layer+1))
                    out,cache3 = batchnorm_forward(out,gamma,beta,self.bn_params[layer])
                out,cache2 = relu_forward(out)
                if self.use_dropout:
                    out,cache4 = dropout_forward(out,self.dropout_param)
                input = out
                if self.use_dropout:
                    toadd = [cache1,cache2,cache4]
                else:
                    toadd = [cache1,cache2]
                if self.normalization == 'batchnorm':
                    toadd.append(cache3)
        ...
        da,dw,db = affine_backward(da,caches.pop()[0])
        grads['W'+str(self.num_layers)] = dw + self.reg * self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = db
        loss += self.reg /2 * (np.sum(np.square(self.params['W'+str(self.num_layers)])))
        for i in range(self.num_layers-1,0,-1):   #from num_layers to 1
            cache = caches.pop()
            if self.use_dropout:
                da = dropout_backward(da,cache[2])
            da = relu_backward(da,cache[1])
            if self.normalization == 'batchnorm':
                da,dgamma,dbeta = batchnorm_backward_alt(da,cache[-1])
                grads['gamma'+str(i)],grads['beta'+str(i)] = dgamma,dbeta
            da,dw,db = affine_backward(da,cache[0])
